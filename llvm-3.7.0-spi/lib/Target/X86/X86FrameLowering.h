...
public:
  ...
  std::vector<int> newObjIds;
  std::vector<int> newObjOffs;
  ...
  bool isValidSPIFunction(MachineFunction& MF) const;
  void createWrapper(MachineFunction &MF) override;
  void sanitizeFrame(MachineFunction &MF) override;
  void insertDummy(MachineFunction &MF) override;
  void calculateNumDummy(MachineFunction &MF);
  void createAlignmentPoint(MachineFunction &MF);
  void saveStackPointer(MachineFunction& MF);
  void setSPIAlignmentAuto() override;
...
private:
  ...
  void BuildStackAlignAND(MachineBasicBlock &MBB,
                          MachineBasicBlock::iterator MBBI, DebugLoc DL,
                          uint64_t MaxAlign, bool inPrologue) const;
